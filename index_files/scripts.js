		...
		jQuery(this).children('ul').children('li').hover(
			function() {
				if(!jQuery(this).children('ul').hasClass('focus')) {
					...
					jQuery(this).children('.nb01:first').stop(true, true).animate({ height:'show' }, 100);
				}
				jQuery("#secnavbg").slideDown(100);
			},
			function() {
				jQuery(this).removeClass('focus');
				jQuery(this).children('.nb01:first').stop(true, true).animate({ height:'hide', opacity:'hide' }, 100);
				jQuery("#secnavbg").hide(100);
			}
		);
		jQuery(this).children('ul').children('li').children('.nb01').hover(
			function() {
				jQuery(this).addClass('focus');
			},
			function() {
...
/////////////////////// 首页滚动条处js///////////////////////////////////
jQuery.fn.extend({//添加滚轮事件//by jun
...
jQuery(document).ready(function(){
/*---------------------------------------------------------以下为自定义滚动条样式，设置时注意逗号--------------*/
jQuery(".scroll").jscroll({ W:"7px"//设置滚动条宽度
				,BgUrl:"url(../images/s_bg1.gif)"//设置滚动条背景图片地址
				,Bg:"right 0 repeat-y"//设置滚动条背景图片position,颜色等
				,Bar:{Bd:{Out:"#023274",Hover:"#023274"}//设置滚动滚轴边框颜色：鼠标离开(默认)，经过
					 ,Bg:{Out:"left 0 repeat-y",Hover:"left 0 repeat-y",Focus:"left 0 repeat-y"}}//设置滚动条滚轴背景：鼠标离开(默认)，经过，点击
				,Btn:{btn:false}
