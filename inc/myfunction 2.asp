...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" style="WIDTH: 100%">
  <tr>
  <style>
  .myptotd{width:150px; height:150px; padding:2px; border:1px #c9d5e0 dashed; background:#ffffff;}
  .myptotd2{width:150px; height:150px;padding:2px;  border:1px #ff9900 dashed; background:#ffffff;}
  </style>
    <%
	call page1()
...
<tr>
          <td  align="center" valign="middle"  class="myptotd" onMouseOver="this.className='myptotd2'" 
 onMouseOut="this.className='myptotd'">
		  ...
		  <%
			...
			response.write("<a href=pic_show.asp?id="&oRs("id")&" alt='"&oRs("title")&"' target=_blank ><img border=""0"" src="""&pic&"""   align=center width=165 height=145   onload=""javascript:resizeimg2(this,145,145)"" /></a>")
				 
			%></td>
        </tr>
        <Tr><td height="29" align="center" style="line-height:35px; width:157px; height:35px; font-weight:bold; padding-top:2px; background: url(../images/protitlebg.jpg) center no-repeat;"><% 
			response.write("<a href=pic_show.asp?id="&oRs("id")&" title='"&oRs("title")&"'  target=_blank >"&left(oRs("title"),14)&"</a>")
...
n_page=checkisint(request("page"),1)*9+90
		ProSql = "select top 8 id,title,pic,tag from product where  1=1 "
		if bid<>0 then ProSql = ProSql&" and bid="&bid
		if sid<>0 then ProSql = ProSql&" and sid="&sid
		if pid<>"" then ProSql = ProSql&" and pid like '%"&pid&"%'"
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" style="WIDTH: 100%">
  <tr>
  <style>
  .myptotd{width:142px; height:142px; padding:2px; border:1px #c9d5e0 dashed; background:#ffffff;}
  .myptotd2{width:142px; height:142px;padding:2px;  border:1px #ff9900 dashed; background:#ffffff;}
  </style>
    <%
	call page1()
		for i=1 to oRs.PageSize
...
<tr>
 ...
 onMouseOut="this.className='myptotd'">
		  ...
		  <%
			...
			response.write("<a href=pic_show.asp?id="&oRs("id")&" alt='"&oRs("title")&"' target=_blank ><img border=""0"" src="""&pic&"""   align=center width=140 height=140 onload=""javascript:resizeimg2(this,140,140)"" /></a>")
				 
			%></td>
        </tr>
        <Tr><td height="29" align="center" style="line-height:35px; width:150px; height:35px; font-weight:bold; padding-top:2px; background: url(../images/protitlebg.jpg) center no-repeat;"><% 
			response.write("<a href=pic_show.asp?id="&oRs("id")&"  title='"&oRs("title")&"' target=_blank >"&left(oRs("title"),14)&"</a>")
...
Response.Write "<ul id=""clover168_new"">"
		...
		Response.Write ("<span style=""float:left; width:auto;text-align:left;line-height:18px; height:18px;""><a href=news_show.asp?id="&oRs("id")&" target=_blank  title="""&oRs("title")&""">")		
...
Function Newslist_index(lm,ts,zs,datetype)

		Dim Newstype,NewsSql
		Newstype = checkisint(request("type"),0)
		if Newstype = 0 and lm = 0 then
		NewsSql = "select top "&ts&"  id,title,addtime from web_news  order by id desc"
		elseif lm <> 0 then
		NewsSql = "select top "&ts&"  id,title,addtime from web_news where type = "&lm&" order by id desc"
		else
		NewsSql = "select top "&ts&"  id,title,addtime from web_news where type = "&Newstype&" order by id desc"
		end if
		oRs.PageSize = ts
		oRs.CursorLocation = 3
		oRs.open NewsSql,conn,1,1
		...
		datestr=""
		Response.Write "<LI>"		
		Response.Write ("<a href=news_show.asp?id="&oRs("id")&" target=_blank  title="""&oRs("title")&""">")		
		Response.Write (read_left(oRs("title"),zs)&"</a>")
		Response.Write ("<span>"&Format_Time(oRs("addtime"),datetype)&"</span>")
		Response.Write ("</li>")
		oRs.movenext
		
		if i=5 then response.write "</ul><ul class=""text_news_shuxian"">"
